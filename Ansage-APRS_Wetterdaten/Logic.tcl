...
# expired.
# playtone - 440HZ square wave at volume 500 for 100ms
#
proc send_rgr_sound {} {
  variable sql_rx_id
...
#SEND DTMF CODE 25 
if {$cmd == "25"} {
  set result [catch {exec ping -c 1 44.143.0.10}];
  if {$result == 0} {
     puts "Net check: Internet/Hamnet connected"
     #playMsg "Diverse" "VerbindungHAMNET";
     spellWord $mycall
     playSilence 250;     
     playMsg "Core" "connected"; 
} else {
     puts "Net check: Internet/Hamnet disconnected"
     #playMsg "Diverse" "KeineVerbindungHAMNET";
     spellWord $mycall     
     playSilence 250;
...
#SEND DTMF CODE 26
  if {$cmd == "26"} {
    puts "Reading 1-wire sensor for repeater temperature"
...
#SEND DTMF CODE 27
  if {$cmd == "27"} {
      puts "Read WX-data from file /tmp/wx_oe9xvi.tcl";
 ...
 if {$cmd == "28"} {
      playSilence 250;
      playMsg "Diverse" "div_audio";
      return 1
}

 if {$cmd == "29"} {
      playSilence 250;
      playMsg "Diverse" "Clubabend_903";
      return 1
}
